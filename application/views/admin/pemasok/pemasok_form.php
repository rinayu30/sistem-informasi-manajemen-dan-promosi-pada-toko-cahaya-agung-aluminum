...
<?php $this->load->view('templates_adm/sidebar')?>
        ...
        <a href="<?=site_url('admin/pemasok')?>" class="btn btn-primary btn-sm"><i class="fa fa-undo fa-sm"></i> Kembali</a>
          <!-- Page Heading -->
          ...
          <div class="card shadow mb-4">  
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Form <?=ucfirst($page)?> Pemasok</h6>
            </div>
                <div class="card-body">
                <div class="box-body table-responsive">
                <div class="row">
                    <div class="col-md-4 offset-4">
                    <form action="<?=site_url('admin/pemasok/proses')?>" method="post">
                        <div class="form-group <?=form_error('nama') ? 'has-error' : null?> ">
                        <label for="nama">Nama Pemasok*</label>
                        <input type="hidden" name ="id" value="<?=$row->id_pemasok?>">
                        <input type="text" name="nama" value="<?=$row->nama_pemasok?>" class="form-control" required>
                        <?=form_error('nama')?>
                        </div>

                        <div class="form-group <?=form_error('kontak') ? 'has-error' : null?>">
                        <label for="kontak">Nomor Kontak *</label>
                        <input type="number" name="kontak" value="<?=$row->kontak?>"class="form-control" required>
                        <?=form_error('kontak')?>
                        </div>
                        ...
                        <textarea name="ket" value="<?=$row->keterangan?>" class="form-control"><?=$row->keterangan?></textarea>
                        </div>
                        <br>
